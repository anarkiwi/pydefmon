"""DefmonPlayer tests for the pitch-slide oscillator.
...
trajectories match LUT-derived predictions.
...
calls ``clear_song_table()`` BEFORE ``set_dl``. Reversing the order
silently zeroes every DL byte; the cascade then refetches every frame
...
def _render_voice_freq(
    ...
    (freq_lo, freq_hi) on ``voice``.

    Frame model: defMON's $1147 row-advance defers the row read by 1
    NMI after timer underflow (per pydefmon.defmon_player Bug-2 fix).
    For these synthetic-prg tests, the player's first NMI emits the
    pre-init zeros (observed[0]); the second NMI is a "warmup" where
    arranger has advanced but no row has been read yet, emitting
    NOTE_PITCH[0]+pb on freq and zeros elsewhere; from the third NMI
    onward the cascade has fired and steady-state output appears. We
    discard the warmup NMI so observed[1+] continues to capture
    cascade-applied state — matching the test assertions that predate
    the bug-2 timing correction.
    """
    ...
    # Run n_frames+1 NMIs; index 1 is the deferred-read warmup, skipped.
...
def _render_voice_pw(
    ...
    """Per-frame emit-time (pulse_lo, pulse_hi) on ``voice``. See
    :func:`_render_voice_freq` for the frame model."""
    player = DefmonPlayer(song)
