"""DefmonPlayer tests for the layer-1 sidcall cascade.
...
Each test builds a minimal synthetic .prg that exercises one
specific layer-1 behaviour and asserts the observed register stream
matches the predicted trajectory.

``clear_song_table()`` gotcha: it wipes $1B00..$1EFF (including the
DL region), so ``_build_layer1_song`` below calls it BEFORE any
``set_dl``.
"""
