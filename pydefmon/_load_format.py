"""defMON $D6C9 LOAD-time codec (decode + encode).

Disassembled from the defMON loader's $D6C9-$D74B body and verified
byte-faithful against a 9-tune corpus. Round-trips on-disk .prg
files through the same byte grammar the real defMON loader uses.

Public surface:

* ``decode_load_stream(body, src_end_addr, src_floor, dest_start)``
  parses a PRG body and returns ``{addr: byte}`` writes. Inverse of
  the on-chip $D6C9 decoder.
* ``encode_ram_block(ram, base_addr)`` builds a complete PRG (load
  address + body) that, when LOADed by defMON, writes ``ram`` to
  ``base_addr..base_addr+len(ram)-1``. Greedy backward emission;
  body size is fixed at ``cost_sum + 6``.

Both functions live in one module because they are inverses; the
caller never wants one without the other.

Path grammar (per iteration, ``b{0,1,2}`` are read at src+{0,1,2}):

* Path A (cost 1, b1 != $FF)                          emit b2; src -= 1
* Path B (cost 2, b1 == $FF, b2 == $FF)               emit $FF; src -= 2
* Path C (cost 1, b1 == $FF, b2 != $FF, b0 == $FF)    emit b2; src -= 1
* Path D (cost 3, b1 == $FF, b2 != $FF, b0 != $FF)    fill (b2+2) × b0; src -= 3
"""
